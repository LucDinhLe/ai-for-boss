...
!macro Support ACTION
  nsExec::ExecToStack '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\install-support.ps1" -Action ${ACTION} -Root "$INSTDIR" -Version "${VERSION}" -Manifest "$PLUGINSDIR\payload-manifest.json" -Desktop "$DESKTOP" -StartMenu "$SMPROGRAMS"'
  Pop $Result
  Pop $Detail
  DetailPrint "$Detail"
  ${If} $Result != 0
...
Section "Install"
  ...
  DetailPrint "Đang kiểm tra thư mục và dung lượng cài đặt..."
  ...
  ${EndIf}
  SetOutPath "$PLUGINSDIR"
  DetailPrint "Đang xác minh các tệp vừa chép. Vui lòng chờ hoàn tất..."
  !insertmacro Support Commit
  Goto verified_install
already_installed:
  DetailPrint "Đang kiểm tra phiên bản đã cài. Vui lòng chờ hoàn tất..."
...
verified_install:
  ...
  ${If} ${Errors}
    ...
    Abort
  ${EndIf}
  DetailPrint "Đang tạo lối tắt và mục gỡ cài đặt..."
