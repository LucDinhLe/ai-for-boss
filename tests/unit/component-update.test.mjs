...
async function fixture() {
  ...
  for (const [kind, items] of Object.entries(files)) {
    ...
    const url = 'https://github.com/LucDinhLe/ai-for-boss/releases/download/test/' + kind + '.zip';
...
test('repository migration preserves signed legacy receipts but rejects lookalike repositories', async () => {
  const f = await fixture(), legacy = structuredClone(f.manifest);
  legacy.components.ui.url = legacy.components.ui.url.replace('/ai-for-boss/', '/ai-for-boss-preview/');
  assert.equal(verifyUpdateEnvelope(f.envelope(legacy), f.publicKey).version, legacy.version);
  for (const repo of ['ai-for-boss-copy', 'ai-for-boss-preview-copy']) {
    const foreign = structuredClone(f.manifest);
    foreign.components.ui.url = foreign.components.ui.url.replace('/ai-for-boss/', '/' + repo + '/');
    assert.throws(() => verifyUpdateEnvelope(f.envelope(foreign), f.publicKey));
  }
});
