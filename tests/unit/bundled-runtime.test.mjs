...
test("a packaged app resolves its own runtime before anything on the host", () => {
  const packaged = fs.mkdtempSync(path.join(process.env.RUNNER_TEMP ?? os.tmpdir(), "aifb-resources-"));
  try {
...
test("an explicit runtime override still wins, and a missing package still falls back", () => {
  const empty = fs.mkdtempSync(path.join(process.env.RUNNER_TEMP ?? os.tmpdir(), "aifb-empty-"));
  try {
