...
## Bộ chạy sống qua các lần đóng cửa sổ — 2026-09-13
Số đo từ log của lõi ngày 13/09/2026: khởi động lạnh 84 giây (riêng nạp 13 plugin của lõi 63,6 giây), rồi vỏ tự thêm 56 giây bằng hai lượt khởi động lại để đăng ký hai plugin của mình, tổng 2 phút 20 giây tới lúc dùng được; trong khi khởi động lại lúc máy đã ấm chỉ mất 4 tới 10 giây. Product Owner chốt cho Gateway chạy nền: đóng cửa sổ thì ẩn và giữ bộ chạy, biểu tượng khay giữ ứng dụng sống, chỉ Thoát hẳn mới tắt. Đổi lại là một tiến trình Node chiếm bộ nhớ khi không dùng, và người dùng có ô đánh dấu để tắt chế độ này. Kèm theo: gộp hai lượt đăng ký plugin thành một patch một lần khởi động lại, chia sẻ lượt đọc `models.list` đang bay trên kênh thiết lập, và ghi dòng thời gian khởi động ra `aifb-startup-timeline.jsonl` để lần sau đo thay vì đoán. Chi tiết: `docs/feature-specs/0059-startup-speed-and-background.md`.

## Không tự viết đường đăng nhập Antigravity — 2026-09-13
Một ứng dụng khác trên máy Product Owner kết nối được Antigravity bằng OAuth, nên câu hỏi đặt ra là AI for Boss có nên làm theo. Lõi OpenClaw đã gỡ hẳn provider `google-antigravity` và plugin `google-antigravity-auth` như một thay đổi phá vỡ, với lý do ghi trong tài liệu nhà cung cấp Google: Google chấm dứt đăng nhập tài khoản cá nhân cho Gemini CLI từ 18/06/2026, và điều khoản Antigravity cấm công cụ bên thứ ba truy cập dịch vụ qua OAuth của họ. Quyết định: giữ nguyên D-0022, vỏ không tự viết đường xác thực nào lõi không có. Trang Nhà cung cấp vẫn hiện Antigravity trong danh mục lõi báo lên, nhưng đường kết nối cho Google là API key AI Studio và Vertex AI. Bật lại OAuth chỉ khi lõi đưa nó trở lại.

## Một nhà cung cấp, nhiều tài khoản, thứ tự do người dùng chọn — 2026-09-13
Product Owner đối chiếu với một ứng dụng khác và yêu cầu trang Nhà cung cấp gọn và rõ như bên đó. Rà lại thì lõi OpenClaw đã có đủ: `models.authStatus` báo từng hồ sơ đăng nhập kèm loại, tình trạng, hạn và mức dùng, còn `auth.order` là trường lõi tự đọc khi định tuyến và xoay vòng. Quyết định: vỏ chỉ vẽ lại những gì lõi báo và ghi thứ tự vào đúng trường của lõi, không dựng bảng thứ tự riêng. Việc ghi đi qua một lời gọi cố định của host (`setAuthOrder`) có đối chiếu id với lõi và đọc lại sau khi vá; `config.*` vẫn không phải phương thức giao diện gọi được. Thứ tự đã lưu mà nhắc tới tài khoản không còn thì id đó rơi ra chứ không che tài khoản đang dùng được. Chi tiết: `docs/feature-specs/0060-provider-accounts.md`.

## Vỏ dọn đường dẫn plugin chết trước khi gọi bộ chạy — 2026-09-13
beta38 không mở được trên máy Product Owner. `openclaw.json` vẫn trỏ hai plugin của vỏ vào `versions\0.0.5-beta.37\resources\...`, thư mục mà bộ cài beta38 đã dọn; lõi từ chối toàn bộ tệp cấu hình chứ không bỏ qua một dòng, nên Gateway không lên, mà `prepareHostPlugins` (thứ lẽ ra sửa đường dẫn) lại chỉ chạy sau khi Gateway lên. Vòng lặp không thoát được, và lỗi thuộc về đợt spec 0059. Quyết định: trước khi dựng `GatewaySupervisor`, host đọc `openclaw.json` và bỏ mọi đường dẫn trong `plugins.load.paths` không còn tồn tại trên đĩa, ghi tạm rồi đổi tên, chỉ đụng đúng trường đó, và im lặng bỏ qua nếu tệp không đọc hay không ghi được. Danh sách về rỗng là kết quả hợp lệ vì đường đăng ký sẵn có sẽ ghi lại đúng bản đang chạy. Chi tiết: `docs/feature-specs/0061-stale-plugin-paths.md`.

## Bộ cài nói bằng mã, lời tiếng Việt nằm trong bộ cài — 2026-09-13
Hộp thoại gỡ beta38 hiện "B???n n??y ??ang ???????c d??ng": PowerShell ghi stdout theo bảng mã console còn `nsExec::ExecToStack` đọc lại theo ANSI, nên dấu tiếng Việt chết trên đường đi; BOM UTF-8 của `.ps1` chỉ bảo đảm lượt đọc, không bảo đảm lượt ghi. Quyết định: `install-support.ps1` chỉ phát mã ASCII, `ai-for-boss.nsi` giữ toàn bộ lời tiếng Việt và ánh xạ mã sang câu, mã lạ in nguyên văn để không mất thông tin chẩn đoán. Cùng lúc sửa nội dung câu "đang được dùng" vốn đã sai từ spec 0059: đóng cửa sổ không còn tắt ứng dụng, phải Thoát hẳn từ khay hệ thống. Không cho bộ cài tự tắt ứng dụng. Chi tiết: `docs/feature-specs/0062-installer-messages.md`.

## Chỉ dùng cấu trúc NSIS đã biên dịch được — 2026-09-14
Bản vá thông báo bộ cài (spec 0062) làm chết bản dựng beta40: `makensis` báo `Invalid command: "${TrimNewLines}"`. Môi trường sửa kho này không cài được NSIS vì kho Ubuntu bị chặn, nên mọi cấu trúc mới trong `.nsi` chỉ lộ lỗi khi một bản dựng thật chạy hai mươi phút rồi chết. Quyết định: tệp `.nsi` chỉ được dùng những dạng LogicLib nó đã biên dịch được (`If`/`ElseIf`/`Else`/`EndIf`) và chỉ hai header `MUI2.nsh` và `LogicLib.nsh`; test đọc tệp giữ luật này. Nhu cầu cắt xuống dòng bị bỏ hẳn bằng cách cho `install-support.ps1` phát mã không kèm ký tự xuống dòng. Cùng luật đó áp cho mọi thay đổi sau trong bộ cài.
