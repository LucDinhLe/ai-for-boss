...
export const zh: Translations = {
  ...
  notifications: {
    ...
    backendOutOfDateBundledMessage:
      '随应用打包的 Hermes 服务与此桌面版本不匹配。请安装最新的 AI for Boss 版本；不支持原地更新该服务。',
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'AI for Boss 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Hermes 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
    ...
    model: {
      ...
      advisorDesc: '独立模型会在 Hermes 回复前检查计划、重大调整或重复错误，以及最终结果。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      ideaHint: '可选。创建项目时，Hermes 会将此内容保存为主文件夹中的 IDEA.md，供智能体了解目标。',
  ...
  shell: {
    ...
    statusbar: {
      ...
      contextUsagePanel: {
        ...
        compactAt: (tokens, percent) => `Hermes 将在 ${tokens} 时压缩（当前路由的 ${percent}%）`,
        compactNow: '已达到压缩阈值',
  ...
  preview: {
    ...
    web: {
      ...
      connector: {
        ...
        enable: '启用',
        disable: '关闭',
        currentSite: 'Hermes Browser 网站',
        createCode: '创建一次性代码',
        pairingTitle: '配对浏览器扩展',
        pairingInstructions: '在 Chrome 或 Edge 的同一网站打开 Hermes Connector，预览 Cookie 后输入此代码。',
        copy: '复制',
        ...
        expiry: '最晚过期时间',
        sessionOnly: '关闭会话时',
        importWarning: '导入可能替换 Hermes Browser 中标识相同的 Cookie。分区 Cookie 会被跳过。',
