...
export const zh: Translations = {
  ...
  boot: {
    ready: 'AI for Boss 桌面版已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败：${message}`,
    steps: {
      connectingGateway: '正在连接桌面网关',
      loadingSettings: '正在加载 AI for Boss 设置',
      loadingSessions: '正在加载最近会话',
      retryingRemoteBackend: '正在重新连接远程 AI for Boss 后端…',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 AI for Boss 桌面版…'
    },
    errors: {
      backgroundExited: 'AI for Boss 后台进程已退出。',
      backgroundExitedDuringStartup: 'AI for Boss 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 AI for Boss 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    backendOutOfDateBundledMessage:
      '随应用打包的 AI for Boss 服务与此桌面版本不匹配。请安装最新的 AI for Boss 版本；不支持原地更新该服务。',
    installMethodUnsupportedTitle: '不受支持的安装方式',
    updateHermes: '更新 AI for Boss',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 AI for Boss。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'AI for Boss 正在等待你的回应。',
      turnDoneTitle: 'AI for Boss 已完成',
      turnDoneBody: '',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 AI for Boss 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 AI for Boss 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      disable: '禁用',
      required: '必需',
      requiredDescription: '这是 AI for Boss 必需的内置功能，会始终保持启用。',
      failed: '失败',
      empty: '尚未安装桌面插件。',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 AI for Boss 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 AI for Boss 后端以便在桌面端管理此插件。',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 AI for Boss 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'AI for Boss 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'AI for Boss 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
        ...
        plugin: {
          label: '插件通知',
          description: 'AI for Boss 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 AI for Boss 跟随系统设置。',
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，AI for Boss 也能回应你的消息。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 AI for Boss 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 AI for Boss，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 AI for Boss 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'AI for Boss 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'AI for Boss 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'AI for Boss 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'AI for Boss 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      heading: 'AI for Boss',
      ...
      license: '许可证',
      licenseValue: 'MIT 许可证',
      communityUpdateChannel: 'AI for Boss 渠道 · GitHub Releases',
      bundleOutOfSync: '应用构建版本过旧',
      bundleOutOfSyncDesc:
        'AI for Boss 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Agents)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      ...
      notifyOnlyDesc:
        'AI for Boss 只会提醒新版本，不会自行下载或安装；请从 GitHub Releases 下载安装包并核对校验值。',
      checksumLine: (filename, sizeMb, sha256) => `${filename} · ${sizeMb} MB · SHA-256 ${sha256}`,
      lastChecked: age => `上次检查:${age}`,
      justNowSuffix: ' · 刚刚',
      automaticUpdates: '自动更新',
      automaticUpdatesDesc: 'AI for Boss 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 AI for Boss 即可发送提示。',
    ...
    connections: {
      ...
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 AI for Boss 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 AI for Boss 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Hermes Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 AI for Boss 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 AI for Boss 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'AI for Boss 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 AI for Boss 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      envOverrideTitle: '环境变量正在控制此桌面会话。',
      envOverrideDesc: '取消设置 HERMES_DESKTOP_REMOTE_URL 和 HERMES_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      modeTitle: '连接模式',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 AI for Boss 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 AI for Boss 后端。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'AI for Boss 将使用已保存设置重新连接（界面保持打开）。',
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'AI for Boss 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshHermesPathTitle: 'AI for Boss 路径（可选）',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 AI for Boss`,
      sshIncompleteHost: '连接前请输入 SSH 主机。',
      sshErrUnreachable: '无法通过 SSH 连接到该主机。请检查主机、端口和网络。',
      sshErrAuth:
        'SSH 认证失败。请将密钥加载到 ssh-agent（ssh-add），或在 ~/.ssh/config 中设置 IdentityFile——AI for Boss 以非交互方式运行 ssh。',
      ...
      sshErrPlatform: '不支持的远程平台。AI for Boss 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
      sshErrTimeout: 'SSH 连接超时。主机可能无法访问或处于休眠状态。',
      sshErrUpdateRequired: '使用 Desktop SSH 连接前，请更新远程主机上的 AI for Boss。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 AI for Boss。请检查下方的完整配置——它来自该链接，而非 AI for Boss。',
    ...
    model: {
      ...
      advisorTitle: '监督模型（Advisor）',
      advisorDesc: '独立模型会在 AI for Boss 回复前检查计划、重大调整或重复错误，以及最终结果。',
      advisorEnabled: '启用监督模型',
      advisorCost: '启用后，每个检查点都会增加一次模型调用、延迟和费用。',
      advisorModel: '监督模型',
      auxiliaryTitle: '辅助模型',
    ...
    providers: {
      ...
      googleAccount: {
        ...
        connected: '已连接',
        tierLabel: '等级',
        caveat: '与 Gemini CLI 走同一入口。Google 可能更改或关闭它；这不是 AI for Boss 官方提供商。',
        errorPrefix: 'Google 登录问题',
        projectPlaceholder: 'Google Cloud 项目 ID（例如 my-project-123456）',
        projectSave: '保存项目'
      },
      haveApiKey: '改用 API 密钥？',
      intro: '使用订阅登录，无需复制 API 密钥。AI for Boss 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 AI for Boss 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
  ...
  skills: {
    ...
    archiveDescription: '技能将被归档，可通过 AI for Boss 技能管理工具恢复。',
    archiveFailed: '无法归档技能',
    defaultProfile: 'AI for Boss（默认）',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 AI for Boss 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 AI for Boss 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 AI for Boss 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 AI for Boss 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 AI for Boss 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `AI for Boss ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
    ...
    maintenance: {
      ...
      revealBackup: '显示备份',
      importBackup: '恢复备份',
      importBackupDesc: '选择 AI for Boss 备份压缩包并恢复配置、记忆、技能与会话',
      importBackupConfirm: '要恢复此备份吗？同名的现有 AI for Boss 数据将被替换，安装文件和应用文件会保留。',
      importBackupRestart: '恢复完成。请重新启动 AI for Boss 以加载所有恢复的数据。',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 AI for Boss 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 AI for Boss 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 AI for Boss 指向该 URL。',
      ...
      weixin:
        '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。AI for Boss 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 AI for Boss 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    connectGateway: '连接另一个 AI for Boss 网关…',
    actions: '操作',
    color: '颜色…',
    colorFor: '颜色',
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 AI for Boss 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      review: '检查定时任务',
      saveFailed: 'AI for Boss 未保存该模型更改。'
    },
    search: '搜索定时任务…',
    loading: '正在加载定时任务…',
    states: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。AI for Boss 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      sectionLabel: '项目',
      manageDescription: '集中管理相关文件夹、会话和使用量。',
      ...
      foldersLabel: '文件夹',
      ideaLabel: '想法',
      ideaPlaceholder: '描述项目目标和主要想法。',
      ideaHint: '可选。创建项目时，AI for Boss 会将此内容保存为主文件夹中的 IDEA.md，供智能体了解目标。',
      ideaSaveFailed: '项目已创建，但无法在主文件夹中保存 IDEA.md。',
      ideaGenerate: '生成想法',
      ideaGenerating: '生成中…',
      ideaShuffle: '随机模板',
      noFolders: '尚未添加文件夹。',
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 AI for Boss 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 AI for Boss 中移除已保存的项目。会话、文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      ...
      worktreeStaleBackend: '请更新 AI for Boss 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 AI for Boss…',
    placeholderReconnecting: '正在重新连接 AI for Boss…',
    ...
    attachUrlDesc: 'AI for Boss 将抓取该页面并作为本回合的上下文。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: '让 AI for Boss 提交并开 PR',
  ...
  updates: {
    stages: {
      ...
      pull: '马上完成…',
      pydeps: '收尾中…',
      update: '正在更新 AI for Boss…',
      rebuild: '正在重新构建桌面应用…',
      restart: '正在重启 AI for Boss…',
      done: '更新完成',
      manual: '从终端更新',
      guiSkew: '请更新桌面应用',
      error: '更新已暂停'
    ...
    unsupportedMessage: '此版本的 AI for Boss 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 AI for Boss 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 AI for Boss 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 AI for Boss，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 AI for Boss 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 AI for Boss 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'AI for Boss 更新器会在自己的窗口中接管，并在完成后自动重新打开 AI for Boss。更新期间请不要自行重新打开 AI for Boss。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 AI for Boss 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 AI for Boss 会自动重新打开。',
    errorTitle: '更新未完成',
    errorBody: '没有数据丢失。你可以现在重试。',
    blockerTitle: '关闭本地预览以更新 AI for Boss？',
    blockerBody: 'AI for Boss 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 AI for Boss',
    foreignBlockerBody: 'AI for Boss 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'AI for Boss 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  install: {
    ...
    oneTimeTitle: 'AI for Boss 需要一次性安装',
    ...
    setupChoiceTitle: '设置 AI for Boss',
    setupChoiceDesc: '将此应用连接到你已运行的 AI for Boss 网关，或在这台电脑上本地安装 AI for Boss。',
    connectExistingTitle: '连接到现有 AI for Boss',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 AI for Boss',
    installLocalDesc: '下载 AI for Boss，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 AI for Boss 后重试。',
    remoteSetupTitle: '连接到现有 AI for Boss',
    remoteSetupDesc: '输入网关 URL。AI for Boss 会检测需要令牌还是浏览器登录。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '使用 Hermes 网关的基础 URL；远程地址请包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: '正在检测网关认证方式...',
    probeError: '无法连接到该 AI for Boss 网关。',
    identityProvider: '你的身份提供方',
    authTitle: '认证',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 AI for Boss',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 AI for Boss CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 AI for Boss 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。AI for Boss 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    workProfile: {
      title: '设置你的工作',
      description: '选择工作领域，AI for Boss 会从本地目录建议少量已安装的 Skill。',
      areasLabel: '工作领域',
      areas: {
        research_learning: '研究与学习',
        leadership_business: '领导与商业',
    ...
    headerTitle: '开始设置 AI for Boss',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'AI for Boss 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 AI for Boss…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 AI for Boss 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 AI for Boss 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — AI for Boss 会自动连接',
    ...
    authorizeThere: '请在那里授权 AI for Boss。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 AI for Boss，连接会自动完成，无需复制或粘贴。`,
  ...
  shell: {
    ...
    gatewayMenu: {
      ...
      restartGateway: '重启',
      stopGateway: '停止',
      forceStopGateway: '强制停止',
      forceStopUnavailable: '此网关来源不支持强制停止。',
      viewLogs: '查看日志',
      runDoctor: '运行诊断',
      checkHealth: '检查健康状态',
      stopConfirmTitle: '停止此网关？',
      stopConfirmBody: profile => `配置文件“${profile}”的活动会话可能会中断。聊天和设置将被保留。`,
      stopSharedConfirmBody: '此共享网关服务多个配置文件。活动会话可能会中断。聊天和设置将被保留。',
      actionFailed: action => `${action}失败。`,
      actionTimedOut: '操作仍在运行，但等待已超时。请先检查状态，再重试。',
      lifecycleManagedBy: profile => `此网关为共享网关，只能从配置文件“${profile}”控制其生命周期。`,
      lifecycleOwnerUnknown: '后端无法确认网关归属，因此生命周期控制不可用。',
      sharedLifecycleWarning: '此网关由多个配置文件共享。重启或停止会影响所有正在服务的配置文件。',
      statusLoadFailed: '无法加载网关状态。',
      logsEmpty: '暂无网关日志。',
      logsLoadFailed: '无法加载网关日志。',
      doctorStarted: '诊断已启动。',
      healthHealthy: '网关运行正常。',
      healthUnhealthy: '网关健康检查失败。',
      openSystem: '打开系统面板',
    ...
    statusbar: {
      ...
      contextUsagePanel: {
        ...
        compactAt: (tokens, percent) => `AI for Boss 将在 ${tokens} 时压缩（当前路由的 ${percent}%）`,
        compactNow: '已达到压缩阈值',
        compactionCount: count => `压缩次数：${count}`,
        costActual: amount => `供应商报告的会话费用：${amount} USD`,
        costDisclaimer: '按公开 API 价格估算；工具、区域和合约费用可能不同。',
        costEstimated: amount => `会话预估费用：${amount} USD`,
        costIncluded: '已包含在当前订阅中',
        costReference: amount => `API 等值参考：${amount} USD — 仅供参考，当前未计费`,
        costReferenceCompact: amount => `API 等值：${amount} USD · 未计费`,
        costTitle: '会话费用',
        costTokens: (input, output, cache) => `${input} 输入 · ${output} 输出 · ${cache} 缓存`,
        costUnknown: '此路由暂无公开价格',
        conversationContext: tokens => `对话上下文：${tokens}`,
        effectiveCapacity: (tokens, percent) => `当前路由限制：${tokens} · 已用 ${percent}%`,
        empty: '暂无上下文数据',
        estimated: '估算值',
        loading: '正在加载明细…',
        measured: '实测值',
        modelLabel: model => `工作模型：${model}`,
        logicalHistory: tokens => `逻辑历史：${tokens}`,
        percentFull: percent => `已用 ${percent}%`,
        publishedCapacity: (tokens, source) => `官方容量：${tokens} · ${source}`,
        quotaRemaining: (provider, percent) => `供应商配额（${provider}）：剩余 ${percent}%`,
        quotaReset: value => `配额重置：${value}`,
        quotaUnavailable: '供应商配额：暂无数据',
        remaining: tokens => `官方窗口剩余：${tokens}`,
        sourceAnthropic: 'Anthropic',
        sourceOpenAI: 'OpenAI',
        sourceRuntime: '当前路由',
        systemBackground: tokens => `系统 + 后台：${tokens}`,
        title: '上下文用量',
        tokenSummary: (used, max) => `${used} / ${max} Tokens`,
        tokensUntilCompact: tokens => `距离 AI for Boss 压缩还有 ${tokens}`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。AI for Boss 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'AI for Boss 正在重启...',
      askRestart: '让 AI for Boss 重启服务器',
      lookingRestart: taskId => `AI for Boss 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'AI for Boss 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `AI for Boss 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'AI for Boss 无法重启服务器。',
      stillWorking: 'AI for Boss 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
      ...
      connector: {
        openButton: '使用 Chrome 或 Edge 会话',
        title: 'AI for Boss Connector',
        description: '通过官方 Chrome 或 Edge 配套扩展传输当前网站的 Cookie。',
        loading: '正在检查连接器…',
        officialExtension: '官方配套扩展',
        trustVerified: '已验证扩展摘要和权限策略。',
        trustFailed: '扩展验证失败。请保持连接器关闭。',
        openFolder: '打开文件夹',
        extensionId: '扩展 ID',
        digest: 'SHA-256',
        enabled: '连接器已启用',
        disabled: '连接器已关闭',
        enableDescription: '启用后可进行短时本机配对；每个网站仍需你的确认。',
        enable: '启用',
        disable: '关闭',
        currentSite: 'AI for Boss Browser 网站',
        createCode: '创建一次性代码',
        pairingTitle: '配对浏览器扩展',
        pairingInstructions: '在 Chrome 或 Edge 的同一网站打开 AI for Boss Connector，预览 Cookie 后输入此代码。',
        copy: '复制',
        ...
        expiry: '最晚过期时间',
        sessionOnly: '关闭会话时',
        importWarning: '导入可能替换 AI for Boss Browser 中标识相同的 Cookie。分区 Cookie 会被跳过。',
        confirmImport: (count, hostname) => `为 ${hostname} 导入 ${count} 个 Cookie`,
        importing: '正在安全等待并导入…',
        imported: (count, hostname) => `已为 ${hostname} 导入 ${count} 个 Cookie。`,
        skippedUnsupported: count => `已跳过 ${count} 个不支持的分区 Cookie。`,
        importedSessions: '已导入的网站会话',
        noImports: '暂无已导入的网站会话。',
        importRecord: (count, time) => `${count} 个 Cookie · ${time}`,
        revoke: '删除',
        revokeWarning: '删除仅移除记录的 Cookie 标识。网站刷新出的同标识 Cookie 也会被删除。',
        error: code => `连接器无法继续（${code}）。Cookie 值未写入日志。`,
        close: '关闭'
      }
    }
  },
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '显示更早的消息',
      loadingResponse: 'AI for Boss 正在加载回复',
    ...
    approval: {
      gatewayDisconnected: 'AI for Boss 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。AI for Boss 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'AI for Boss 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'AI for Boss 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 AI for Boss 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 AI for Boss 桌面版以保存图片',
