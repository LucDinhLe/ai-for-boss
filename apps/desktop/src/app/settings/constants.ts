...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before AI for Boss stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters AI for Boss can read from one file request.',
  approvals: {
    mode: 'How AI for Boss handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When AI for Boss updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
