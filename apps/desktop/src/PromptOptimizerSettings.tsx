...
type Config={enabled:boolean;daily:boolean;model:Model;reviewer:Model;maxCalls:number;cases:unknown[]};
...
export default function PromptOptimizerSettings({ready,models}:{ready:boolean;models:Model[]}){
 ...
 useEffect(()=>{let alive=true;
  const load=async(initial=false)=>{try{const value=await manage<State>({action:'optimizer-status'});if(!alive)return;setState(value);
   if(initial&&value.settings){const s=value.settings;setEnabled(s.enabled);setDaily(s.daily);setWorker(key(s.model));setReviewer(key(s.reviewer));setMaxCalls(s.maxCalls);setCases(JSON.stringify(s.cases,null,2));}
 ...
 const action=async(name:string,revision?:string)=>{setPending(true);setError('');try{
  ...
  if(name==='optimizer-save'){
   ...
   settings={enabled,daily,model:{id:w.id,provider:w.provider},reviewer:{id:r.id,provider:r.provider},maxCalls,cases:JSON.parse(cases)};
  }
 ...
 return <div className="settings-card"><h2>Tối ưu kế hoạch theo model</h2>
  ...
  <fieldset disabled={!ready||pending||Boolean(state?.busy)} className="model-settings-fields">
   ...
   <label>Bài đánh giá JSON — thay ví dụ bằng nhiệm vụ đại diện của anh<textarea rows={12} value={cases} onChange={e=>setCases(e.target.value)} spellCheck={false}/></label>
   <p>holdout: true là bài kiểm tra không đưa vào bước đề xuất. Mỗi yêu cầu cần mô tả kết quả có thể kiểm tra; giữ ít nhất một bài mỗi loại.</p>
