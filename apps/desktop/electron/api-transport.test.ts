...
describe('keep-alive agent pools', () => {
  ...
  it('retires idle sockets before uvicorn (5 s keep-alive) closes them', () => {
    for (const agent of [jsonAgentFor('http:'), jsonAgentFor('https:'), downloadAgentFor('http:')]) {
      expect((agent as unknown as { options: { timeout?: number } }).options.timeout).toBe(FREE_SOCKET_TIMEOUT_MS)
    }

    expect(FREE_SOCKET_TIMEOUT_MS).toBeLessThan(5_000)
  })
})
