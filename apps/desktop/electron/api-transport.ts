...
// Hermes Vietnamese (salvaged from upstream v2026.9.7, see docs/patch-ledger.md):
// the local backend is uvicorn with its default 5 s keep-alive. Node's pooled
// sockets used to live exactly as long, so a request sent ~5 s after the
// previous one raced the server's close and died with ECONNRESET / socket hang
// up (reproduced 2026-09-23: 1 in 18 calls at a 4.99 s cadence). Retire idle
// sockets strictly before the server does; the retry policy below covers the
// residual race.
export const FREE_SOCKET_TIMEOUT_MS = 4_000

// JSON pool: many small concurrent calls (session lists, config, prompts).
const HTTP_JSON_AGENT = new http.Agent({ keepAlive: true, maxSockets: 50, timeout: FREE_SOCKET_TIMEOUT_MS })
const HTTPS_JSON_AGENT = new https.Agent({ keepAlive: true, maxSockets: 50, timeout: FREE_SOCKET_TIMEOUT_MS })
...
const HTTP_DOWNLOAD_AGENT = new http.Agent({ keepAlive: true, maxSockets: 8, timeout: FREE_SOCKET_TIMEOUT_MS })
const HTTPS_DOWNLOAD_AGENT = new https.Agent({ keepAlive: true, maxSockets: 8, timeout: FREE_SOCKET_TIMEOUT_MS })
