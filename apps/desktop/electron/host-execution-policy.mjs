...
const error = () => new Error('Chưa xác nhận được chế độ duyệt từng lệnh. Chưa gửi yêu cầu đến mô hình.');
...
export class HostExecutionPolicy {
  ...
  async #ensure(key) {
    ...
    if (key !== undefined) {
      ...
      if (session.permissionMode !== 'guarded') {
        const id = session.sessionId;
        await this.request('sessions.patch', { key, expectedSessionId: id,
          expectedPermissionMode: session.permissionMode ?? null, permissionMode: 'guarded' });
        session = (await this.request('sessions.describe', { key }))?.session;
        if (session?.sessionId !== id || session?.key !== key) throw error();
      }
      if (session.permissionMode !== 'guarded') throw error();
    }
    return { verified: true, permissionMode: 'guarded', approval: 'every-command', sandbox: false };
  }
}
