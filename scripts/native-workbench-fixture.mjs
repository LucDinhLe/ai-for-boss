...
async function worker({ root, resources, timeoutMs }) {
  ...
  const record = { kind: "AIFB_NATIVE_WORKBENCH_FIXTURE", recordedAt: new Date().toISOString(),
  ...
  globalThis.window = { aiForBoss: { gateway: { request } } };
  ...
  try {
    await Promise.race([(async () => {
      ...
      await check("thinkingScopeBoundary", async () => {
        ...
        await assert.rejects(request("sessions.patch", { key: sessionKey, agentId: "fixture", thinkingLevel: choice.id }), /missing scope: operator.admin/);
...
async function main() {
  ...
  const output = path.resolve(value("--out") ?? path.join(repoRoot, "artifacts", "reference-workbench", `native-workbench-${Date.now()}.json`));
  const timeoutMs = 360000;
  const { node } = fixtureRuntime(resources);
  const tempParent = realpathSync.native(os.tmpdir());
  const root = realpathSync.native(mkdtempSync(path.join(tempParent, "aifb-native-workbench-")));
  ...
  receipt ??= { kind: "AIFB_NATIVE_WORKBENCH_FIXTURE", realAI: false, pass: false, failures: ["Worker exited without receipt"] };
  receipt.workerExitCode = exitCode; receipt.forcedCleanup = forcedCleanup;
  receipt.pass = receipt.pass && exitCode === 0 && !forcedCleanup;
  if (path.dirname(root) !== tempParent || !path.basename(root).startsWith("aifb-native-workbench-")) throw new Error("Unsafe temporary cleanup path");
