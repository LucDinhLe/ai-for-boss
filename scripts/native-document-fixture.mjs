...
import { fixturePdf, fixtureDocx, fixtureZip } from "./fixture-documents.mjs";
...
const SENTINELS = ["AIFB_COMPOSER_TXT_PAYLOAD_ONE_493715", "AIFB_COMPOSER_TXT_PAYLOAD_ONLY_286041", "AIFB_ZIP_INSIDE_793182"];
const REPLIES = ["AIFB composer fixture: text attachment received.", "AIFB composer fixture: file-only attachment received.", "AIFB ZIP received as data."];
...
async function startModel(apiKey) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (index >= 3) throw new Error("Unexpected extra simulated model request");
      ...
      if (index === 0 && (!shape.documentTextPresent || !shape.untrustedOpeningBeforeText || !shape.untrustedClosingAfterText
        || !shape.externalSourceLabel || !userContents.some((text) => text.includes(PROMPT)))) throw new Error("Native PDF extraction boundaries missing");
...
async function worker({ root, resources, timeoutMs }) {
  ...
  const record = { kind: "AIFB_NATIVE_COMPOSER_FIXTURE", recordedAt: new Date().toISOString(),
    ...
    evidenceScope: "Production Supervisor and public SDK adapters; PDF extraction, DOCX and ZIP transport/extraction observation and session title/rename; no renderer",
  ...
  try {
    await Promise.race([(async () => {
      ...
      const created = await adapter.request("sessions.create", { key: `aifb-composer-${randomUUID()}`, agentId: "fixture",
        model: MODEL_REF, permissionMode: "read-only", thinkingLevel: "off" });
      ...
      for (let index = 0; index < 3; index++) {
        const bytes = index === 0 ? fixturePdf(`${SENTINELS[0]} ${"Synthetic local document for attachment validation. ".repeat(5)}`) : index === 1 ? fixtureDocx(SENTINELS[1]) : fixtureZip({ "notes.txt": SENTINELS[2] });
        const attachment = { type: "file", mimeType: index === 0 ? "application/pdf" : index === 1 ? "application/vnd.openxmlformats-officedocument.wordprocessingml.document" : "application/zip", fileName: index === 0 ? "fixture-report.pdf" : index === 1 ? "fixture-document.docx" : "fixture-archive.zip",
      ...
      const titles = await adapter.request("sessions.list", { agentId: "fixture", includeDerivedTitles: true, limit: 30 });
      const titled = titles.sessions?.find(row => row.key === sessionKey);
      record.titles = { automaticPresent: Boolean(titled?.derivedTitle), derivedTitle: titled?.derivedTitle };
      if (!titled?.derivedTitle?.includes("AIFB_COMPOSER")) throw new Error("Native derived title missing");
      const label = "Rà soát tài liệu thử nghiệm";
      await adapter.request("sessions.patch", { key: sessionKey, label });
      const renamed = await adapter.request("sessions.list", { search: label, includeDerivedTitles: true, limit: 30 });
      record.titles.manualPersisted = renamed.sessions?.some(row => row.key === sessionKey && row.label === label);
      if (!record.titles.manualPersisted) throw new Error("Native rename readback missing");
      if (model.observation.requests !== 3 || model.observation.rejected !== 0) throw new Error("Unexpected simulated model calls");
    })(), cancelled]);
  } catch (error) {
