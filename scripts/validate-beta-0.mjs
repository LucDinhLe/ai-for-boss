...
// Full product preset: D0054 retires the minimal spike's channel-skip flag.
for (const key of [
...
if (EMBEDDING_ENV.OPENCLAW_SKIP_CHANNELS !== '0' || EMBEDDING_ENV.OPENCLAW_SKIP_PROVIDERS !== '0') {
  failures.push('full product must retain native channel/provider runtime configuration');
}
...
// D0056 permits display aliases and priority, but never fabricated auth routes.
const connect = read("apps/desktop/src/connect/ConnectScreen.tsx");
if (/authChoice:\s*["']/u.test(connect)) failures.push('the Connect screen hard-codes an authentication choice');
...
// The public wizard.status has no step. Fetch the next prepared step through
// wizard.next without an answer; no-step is not completion (D-0030).
if (!connect.includes("wizard.next") || !connect.includes('kind: "api-key"')) {
