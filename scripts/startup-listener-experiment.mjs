/** Feature 0013 initial-listener comparison. No product defaults or provider RPCs. */
...
const cases = ["baseline", "listener"];
if (!process.argv.includes("--resources") || !value("--resources")) throw new Error("RESOURCES_REQUIRED");
...
const outputDirectory = path.join(root, "artifacts/native-readiness/startup-listener-experiment");
...
function assertTemporaryHome(directory) {
  ...
  if (parent !== realpathSync.native(os.tmpdir()) || !/^aifb-listener-study-[A-Za-z0-9]+$/u.test(path.basename(actual)))
...
async function worker() {
  ...
  const delayed = name === "listener";
  if (process.env.NODE_DISABLE_COMPILE_CACHE !== undefined || process.env.NODE_COMPILE_CACHE !== undefined)
    throw new Error("UNEXPECTED_CACHE_ENVIRONMENT");
  const home = process.env.HOME;
  if (!home || process.env.USERPROFILE !== home || process.env.PATH !== "" || !path.basename(home).startsWith("aifb-listener-study-"))
  ...
  const record = { case: name, startedAt: new Date().toISOString(), node: process.version, openclaw: version,
    mode: delayed ? "probe-before-initial-sdk-connect" : "immediate-sdk-connect",
    homeFresh: true, osFileCache: "already-warm-or-uncontrolled; not a cold-OS claim",
    sdkAttempts: { chat: [], setup: [] }, sdkStartedMs: null, probeCount: 0, listenerReadyMs: null,
    gatewaySpawnMs: null, chatReadyMs: null, setupReadyMs: null, bothReadyMs: null,
  ...
  const { waitForGatewayListener, probeGatewayListener } = await import("../apps/desktop/electron/startup-listener.mjs");
  class ObservedClient extends GatewayClient {
    constructor(options) {
      const channel = options.clientDisplayName.endsWith("setup") ? "setup" : "chat";
      super({ ...options, hostDeps: { ...options.hostDeps, beforeConnect: () => {
        if (record.sdkAttempts[channel].length < 100) record.sdkAttempts[channel].push(elapsed());
      } } });
    }
  }
  ...
  const chat = new GatewayAdapter({ stateDirectory, appVersion: "0.0.5-beta.3", Client: ObservedClient, logger,
    onStatus: status => {
  ...
  const setup = new SetupChannel({ stateDirectory, appVersion: "0.0.5-beta.3", Client: ObservedClient, logger,
    onStatus: status => {
  ...
  try {
    ...
    if (delayed) {
      record.listenerBarrierResult = await waitForGatewayListener({ port, supervisor,
        shouldStop: () => quitting || record.ownedPids.length > 1,
        timeoutMs: Math.max(1, 220_000 - elapsed()),
        probe: async options => { record.probeCount++; return probeGatewayListener(options); }
      });
      if (record.listenerBarrierResult !== "ready") throw new Error("LISTENER_BARRIER_FAILED");
      record.listenerReadyMs = elapsed();
    }
    record.sdkStartedMs = elapsed();
...
async function runCase(name, home) {
  ...
  console.log(`[listener experiment] START ${name}`);
  ...
  child.on("message", message => {
    ...
    if (message?.kind === "progress") console.log(`[listener experiment] ${name}: ${Math.round(message.elapsedMs / 1000)}s waiting`);
  });
  ...
  console.log(`[listener experiment] END ${name}: both=${record.bothReadyMs ?? "unavailable"}ms failures=${record.failures.length}`);
...
async function orchestrate() {
  ...
  const homes = [mkdtempSync(path.join(realpathSync.native(os.tmpdir()), "aifb-listener-study-")),
    mkdtempSync(path.join(realpathSync.native(os.tmpdir()), "aifb-listener-study-"))].map(assertTemporaryHome);
  const records = [];
  const summary = { recordedAt: new Date().toISOString(), classification: "exploratory-two-runs-not-performance-guarantee",
    sourceCheckpoint: "c0428a3", sourceRuntime: "InternalBuilds/0.0.5-beta.3/resources", hardWallPerRunMs: 240000,
    handshakeBudgetMs: 220000, cleanupReserveMs: 20000, order: cases,
    isolation: "two empty temporary homes; one per condition, same immutable runtime",
  ...
  try {
    for (let index = 0; index < cases.length; index++) records.push(await runCase(cases[index], homes[index]));
  ...
  if (records.length !== 2 || records.some(record => record.failures.length) || summary.failures.length) process.exitCode = 1;
  console.log(`[listener experiment] SUMMARY ${path.join(outputDirectory, "summary.json")}`);
}
