...
for (const kind of ['ui', 'core']) {
  ...
  if (kind === 'core' && reuseRelease) {
    ...
    components[kind] = { ...previous, url: `https://github.com/LucDinhLe/ai-for-boss/releases/download/${pkg.version}/${name}` }; continue;
  }
  ...
  components[kind] = { id, files, bytes: (await stat(target)).size, sha256: digest.digest('hex'),
    url: `https://github.com/LucDinhLe/ai-for-boss/releases/download/${pkg.version}/${name}` };
}
