...
const child = spawn(path.join(root, "node_modules/electron/dist", electron), [path.join(root, "scripts", fixture), ...(interactive ? ['--interactive'] : []), ...(process.argv.includes('--multitasking') ? ['--multitasking'] : []), ...(process.argv.includes('--clipboard-event-only') ? ['--clipboard-event-only'] : []), ...(process.argv.includes('--data-agents') ? ['--data-agents'] : []), ...(process.argv.includes('--trial-release') ? ['--trial-release'] : []), ...(process.argv.includes('--browser-workbench') ? ['--browser-workbench'] : []), `--user-data-dir=${home}`],
